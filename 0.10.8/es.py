try:
    ...
    with open(os.path.join(os_location, "system", "path", "discpath.config"), "r") as path:
        discpath = path.read()
        try:
            discpath = discpath.replace("$login", system_user)
        except:
            discpath = "/mnt"
    ...
    def entry():
        ...
        crod = "}"
        entry = (
            f"\033[1m\033[36m┌─[\033[34mLittle es {systemData.version}\033[36m]─{corg}\033[31m{cd}\033[36m{crod}─(\033[32m~\033[36m)\n"
        ...
        def login():
            ...
            while True:
                ...
                if isGoodUsername == "True":
                    if systemData.database[user]["password"] == hashlib.sha256("".encode()).hexdigest():
                        isGoodPassword = f" "
                    else:
            ...
            for folder in folders:
                ...
                if not os.path.exists(folderpath):
                    if folder in ["system", "boot"]:
                        ...
                        quit()
                    os.mkdir(folderpath)
            if os.path.exists(os.path.join(os_location, "tmp", "debug.temp")):
                debug = True
    ...
    class System:
        ...
        def saveConfig(name):
            ...
            with gamedata.Gopen(os.path.join(userdir, "userdata.config")) as dta:
                dta.open("m")
                dta["password"] = systemData.database[name]["password"]
                dta["commonName"] = systemData.database[name]["commonName"]
                dta["firstname"] = systemData.database[name]["firstname"]
                dta["familyname"] = systemData.database[name]["familyname"]
                dta["birthday"] = systemData.database[name]["birthday"]
            
            with gamedata.Gopen(os.path.join(userdir, "system.config")) as dta:
                dta.open("m")
                dta["lang"] = systemData.database[name]["lang"]
                dta["accountType"] = systemData.database[name]["accountType"]
                dta["devmode"] = systemData.database[name]["devmode"]
                dta["autoAuth"] = systemData.database[name]["autoAuth"]
                dta["debugmode"] = systemData.database[name]["debugmode"]
    ...
    listuser = [
        nom for nom in os.listdir(os.path.join(os_location, "home"))
        if nom != "test"
    ]
    
    if not len(listuser) > 0:
        with open(os.path.join(os_location, "tmp", "firstuser.temp"), "w+"):
            pass
    ...
    while True:
        ...
        elif command == "unlog" and not debug:
        ...
        # change path
            ...
            try:
                if path.startswith("~"):
                    if debug:
                        path = path.replace("~", "sys:/debug:/")
                    else:
                        path = path.replace("~", f"sys:/home:/{username}")
